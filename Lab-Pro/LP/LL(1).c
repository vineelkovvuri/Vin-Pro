...
//Last Modified: Thu Aug 16 2007 9:48:04 PM -- Vineel
...
struct FFTableEntry{
	...
	struct {
		char c;
		int prodID;		//prodID contains the production number whose First and Follow lead to the terminal c ......
	}First[10],Follow[10];
}FFTable[]={{'E'},{'X'},{'T'},{'Y'},{'F'}};
...
void First(int i)
{
	//If Grammer[i].Prod[0] is Non Terminal call First(index) recursively with index as the Grammer[i].Prod[0]'s ID.....
...
void Follow(char X)
{
	...
	for(i = 0; i < nRules; i++)
		...
		if((c = strchr(Grammer[i].Prod,X))!=NULL){
			...
			if(*c >= 'A' && *c <= 'Z'){	//If A -> aBC , then First(C) is added to Follow(B)
				//First of b in A -> aBb 
				if(strchr(t,*c) == NULL){ //Avoids duplication..... 
					temp = GetIndex(*c,0);
					for(j=0;FFTable[temp].First[j].c != '\0';j++)
						if(FFTable[temp].First[j].c != '#'){
							FFTable[idx].Follow[FFTable[idx].nFollow].c = FFTable[temp].First[j].c;
							FFTable[idx].Follow[FFTable[idx].nFollow++].prodID = FFTable[temp].First[j].prodID;
						}
					//If C -> # i.e., first(C) contains epsilon , then effectively(A is) A -> aB so we add Follow(A) to Follow(B)
					if(IsEpsilon(temp)){
						if(Grammer[i].NonTer != X){
							temp = GetIndex(Grammer[i].NonTer,0);
							//Copying Follow(A) to Follow(B)
							for(k = 0 ;k<FFTable[temp].nFollow;k++){
								FFTable[idx].Follow[FFTable[idx].nFollow].c = FFTable[temp].Follow[k].c;					
								FFTable[idx].Follow[FFTable[idx].nFollow++].prodID = FFTable[temp].Follow[k].prodID;
							}
						}
						sprintf(t,"%s%c",t,*c);	
					}
			...
			else if(*c != '\0'){//Terminal..i.e., A -> aBb , so b itself is the follow of B
...
int main()
{
	...
	//Finding the Terminals in the given Grammer....
	for(i=0;i<nRules;i++)
		for(j=strlen(Grammer[i].Prod) - 1; j>=0; j--)
			if((!isupper(Grammer[i].Prod[j])) && Grammer[i].Prod[j] != '#') 
				if(strchr(ParseTable[0] , Grammer[i].Prod[j] ) == NULL)
					sprintf(ParseTable[0],"%s%c",ParseTable[0],Grammer[i].Prod[j]);
	sprintf(ParseTable[0],"%s%c",ParseTable[0],'$');
	...
	for(i=1;i<=nFFTable;i++){
		...
		for(j=0;ParseTable[0][j] != '\0'; j++)
			if(ParseTable[i][j] != -2)printf("%5s | ",Grammer[(int)ParseTable[i][j]].Prod);
			else printf("%5s | ","");
		printf("\n");
	}
	...
	Stack[++top].c = '$';
	Stack[top].terminal = 1;
	Stack[++top].c = Grammer[0].NonTer;
	Stack[top].terminal = 0;
	PrintStack();
	printf("%10s | ",buf);
	...
	while(Stack[top].c != '$')
	{
		if(Stack[top].terminal == 1){
			if(buf[i] == Stack[top].c){
				i++;top--;
				PrintStack();
				printf("%10s | ",&buf[i]);
			}
			...
			if(idx != -2){	//Found a valid Terminal.....
				...
				if(Grammer[idx].Prod[0] != '#')				
					for(j = strlen(Grammer[idx].Prod) - 1;j>=0;j--){
						Stack[++top].c = Grammer[idx].Prod[j];
						Stack[top].terminal = !isupper(Grammer[idx].Prod[j]);
					}
				PrintStack();
				printf("%10s | %c -> %s ",&buf[i],temp , Grammer[idx].Prod);
			}
			else {		//No production exists in the parse table for the given input symbol.....:((		
				printf("\n\n%s is Not a valid language for the given grammer",buf );
				exit(1);
			}
		}
	}
	printf("\n\n%s is %s a valid language for the given grammer",buf, buf[i] != '$' ? "Not" : "");
	return 0;
}
