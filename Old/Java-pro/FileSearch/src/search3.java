/*
 *  This is a file searching program written in Java.
 *  Given a file name the program will locate the file path in the system.
 *
 *         Concepts used
 *         1.Generics
 *         2.Enhanced for loop
 ...
 *  NOTE : This program works only with
 *         JRE >=1.5 and Windows NT(make use of Rundll32.exe and shell32.dll)
 *
 */
//Last Modified: 
// Mon Oct 22 2007 12:26:39 AM 
// 4:41 PM Tuesday, July 12, 2011 
...
//  ==========================================================
    public void mousePressed(MouseEvent e) {    // File Copying starts Here
        try {
            ...
            search3.s.setTitle("File Searching Program - "+"File Copying is in Progress Please Wait...");
            out.transferFrom(in, 0, in.size());    // file copying process
            out.close();
            in.close();
            search3.s.setTitle("File Searching Program - "+"File copying sucessfully completed");
            JOptionPane.showMessageDialog(null, "Sucessfully Copied");
        } catch (Exception ee) {
            JOptionPane.showMessageDialog(
                null,
                h.get(e.getSource())
                + " ERROR : possible reasons\n1.You donot have permissions to copy here \n2.you have not selected an entry\n3.Destination device is not ready");
        }
    }
...
//  ==========================================================
    public void mouseEntered(MouseEvent e) {
        if (e.getSource() instanceof JMenu) {
            search3.s.setTitle("File Searching Program - "+h.get(e.getSource()));    // just for showing the path seleting by the user in the title bar
...
public class search3 extends JFrame implements ActionListener, Runnable, Printable, MouseListener {
    ...
    private JButton          jb1, jb2,  jb4, jb5,  print;
    private JLabel           jl1, jl2, jl3,  jl6, jl7, jl8;
    private JList            jli;
    ...
    private void initiate() {
        c = getContentPane();
        c.setLayout(null);
        setTitle("File Searching Program");
        tk = getToolkit();
        ...
        setBounds((d.width - 840) / 2, (d.height - 500) / 2, 840, 500);
        setResizable(false);
        ...
        if (System.getProperty("java.version").compareTo("1.5") < 0) {
            JOptionPane.showMessageDialog(
                    ...
                    + "\nJava version should be >=1.5\n", "File Searching Program", JOptionPane
                        .INFORMATION_MESSAGE);
            System.exit(-1);
        }
        ...
        int BX = -115, BY = 65; //start of buttons Layout
        
        jl7 = new JLabel("3. Perform: ");
        c.add(jl7);
        jl7.setBounds(BX+135, BY+18, 100, 20);
        
        jb1 = new JButton("Search", new ImageIcon("go.gif"));
        c.add(jb1);
        jb1.setMnemonic('O');
        jb1.setDefaultCapable(true);
        jb1.setBounds(BX+250, BY+20, 110, 20);
        jb1.addActionListener(this);
        jb1.addMouseListener(this);
        jb4 = new JButton("Browse", new ImageIcon("browse.png"));
        c.add(jb4);
        jb4.setMnemonic('B');
        jb4.setBounds(BX+370, BY+20, 100, 20);
        jb4.addActionListener(this);
        jb4.addMouseListener(this);
        jb2 = new JButton("Clear", new ImageIcon("clear.png"));
        c.add(jb2);
        jb2.setMnemonic('L');
        jb2.setBounds(BX+480, BY+20, 100, 20);
        jb2.addActionListener(this);
        jb2.addMouseListener(this);
        jb5 = new JButton("Stop", new ImageIcon("stop.gif"));
        c.add(jb5);
        jb5.setMnemonic('S');
        jb5.setBounds(BX+590, BY+20, 85, 20);
        jb5.addActionListener(this);
        jb5.addMouseListener(this);
        

        jl6 = new JLabel("2. Select drives:");
        //jl6.setDisplayedMnemonic('D');
        //jl6.setLabelFor(jtf1);
        c.add(jl6);
        jl6.setBounds(20, 50, 100, 20);
        ...
        jta2.setFont(new Font("Helvetica", Font.ITALIC|Font.BOLD, 13));
        //jta2.setBackground(new Color(R, G, B));
        //jsp.setBackground(new Color(R, G, B));
        jsp.setBorder(BorderFactory.createTitledBorder(BorderFactory.createBevelBorder(5), "Help",TitledBorder.CENTER,TitledBorder.TOP));
        c.add(jsp);
        jl1 = new JLabel("1. Name of the file:");
...
//        jl6.setBounds(690, 360, 50, 90);
        print = new JButton("Print searched files list", new ImageIcon("print.png"));
        print.setBounds(510, 390, 240, 30);
        print.setMnemonic('P');
        ...
        jl8.setBounds(20, 115, 299, 20);
        jsp = new JScrollPane(jli);
        c.add(jsp);
        jsp.setBounds(20, 141, 300, 170);
        initializepopup();
    ...
    private void pattern(String s) {
        ...
        setTitle("File Searching Program - "+jtf1.getText());
    }
    ...
    public void actionPerformed(ActionEvent e) {
        try {
            ...
            if ((e.getSource() == jb1) || (e.getSource() == jtf1)) {    // go
                ...
                {
                    JOptionPane.showMessageDialog(this,
                    "Enter a valid file name ", "File Searching Program",
            ...
            if (e.getSource() == jb4) {    // browse
				...
				if (jtf1.getText().trim().equals("")) {
                    JOptionPane.showMessageDialog(this,"Enter the name of the file", "File Searching Program",
                    JOptionPane.ERROR_MESSAGE);
                    jtf1.setText("");
                }
                else 
            ...
            if (e.getSource() == jm6) {      // delete
                if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(this, "Are you sure you want to delete it...")) {
                    try {
                        File f = new File((String) jli.getSelectedValue());
    ...
    public void mouseEntered(MouseEvent c) {
        if (c.getSource() instanceof JCheckBox) {
            for (int i = 0; i < drives.length; i++) 
                if (c.getSource() == cb[i]) 
                    jta2.setText("Check this to search in " + drives[i].getPath());

            if (c.getSource() == jcb[0])jta2.setText("Check this to search in hidden folders");
            if (c.getSource() == jcb[1]) jta2.setText("Check this to make search case sensitive");

		}

        // =================================
        if (c.getSource() == jb1)jta2.setText("Click this to start the search");

        if (c.getSource() == jb2)jta2.setText("Click this to clear the results and text fields to their defaults");


        if (c.getSource() == jb4)jta2.setText("Click this to search inside a specified directory");

        if (c.getSource() == jb5)jta2.setText("Click this to stop the searching");

        if (c.getSource() == jl8) jta2.setText("");

        if (c.getSource() == print)jta2.setText("Click this to print results");
        if (c.getSource() == jtf1) jta2.setText("Name of the file to search, Can include regular expressions");

		if (c.getSource() == jtf2) jta2.setText("Number of files searched");

        if (c.getSource() == jtf3)jta2.setText("Number of files matched with the given name");
...
*/
        ...
        if (c.getSource() == jli)jta2.setText("Results matching given file name(file paths). Select any file path and right click to get more advanced options");
    ...
    public void run() {
        ...
        if (selected()) {
            try {
                ...
                JOptionPane.showMessageDialog(this, "Your CD drive(s) is(are) not ready", "File Searching Program",
                                              JOptionPane.ERROR_MESSAGE);
            }
        } else JOptionPane.showMessageDialog(this, "Select atleast one drive to search", "File Searching Program", JOptionPane.ERROR_MESSAGE);
    }
