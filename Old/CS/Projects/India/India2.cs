/*
 * This program extracts the marks of IT(3-2 GVPCOE) from IndiaResults.com  
 *
 *	 							 	 By
...
class India
{
	static string name = "99\">";
	static string stuName = "Name:";
	static string rollNum = "No:";
	...
	public static void Main()
	{
		...
		India ind = new India();
		Console.WriteLine("This program retrieves the marks of III/IV 2nd Sem IT students of Gayatri College Of Engg.........\n");
		...
		while( true ) {
			...
			switch( choice ) {
				...
				case 4:
					...
					break;
				case 5:
						StreamWriter sw = new StreamWriter( "Class Marks 3-2.txt" );
                        Console.SetOut( sw );
						ind.AllStudents();
	...
	private void IndividualStudent( string rollno )
	{
		try {
			...
			//http://jntu.ac.in/results/results_btech.php?Hno=04131a1256&Confirm=Submit&tab=149_32r04&title=B.Tech.+III+Year+II+Sem.+%28RR%29+Regular+May+-2007+&note=&action=confirm
			sr = GetStream( "http://jntu.ac.in/results/results_btech.php?Hno=" + rollno + "&Confirm=Submit&tab=149_32r04&title=B.Tech.+III+Year+II+Sem.+%28RR%29+Regular+May+-2007+&note=&action=confirm" );
			stream = sr.ReadToEnd();
	...
	private void AllStudents()
	{
		for( int i = 0 ; i < noStu ; i++ ) {
			...
			sr = GetStream( "http://jntu.ac.in/results/results_btech.php?Hno=04131a" + ( 1200 + i + 1 ) + "&Confirm=Submit&tab=149_32r04&title=B.Tech.+III+Year+II+Sem.+%28RR%29+Regular+May+-2007+&note=&action=confirm" );
			stream = sr.ReadToEnd();
	...
	{
		int t;
		if( s.Contains( "Record not available..." ) ) { stu.exists = false; return; }

		int pos;

		//Extract Student Roll Number
		pos = s.IndexOf( rollNum ) + rollNum.Length;
		s = s.Substring( pos );
		stu.roll = s.Substring( 0 , s.IndexOf( '<' ) ).Trim();

		//Extract Student Name
		pos = s.IndexOf( stuName ) + stuName.Length;
		s = s.Substring( pos );
		stu.name = s.Substring( 0 , s.IndexOf( '<' ) ).Trim();

		for( int i = 0 ; i < 8 ; i++ ) {

			//Bypass subject code present in HTML
			...
			//bypass credits present in HTML code
			pos = s.IndexOf( name ) + name.Length;
			s = s.Substring( pos );
