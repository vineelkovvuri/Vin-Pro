/*
 * This program extracts the marks of IT(4-1 GVPCOE) from IndiaResults.com  
 *
 *	 							 	 By
...
class India
{
    ...
    public static void Main()
    {
        ...
        India ind = new India();
        Console.WriteLine("This program retrieves the marks of IV/IV 1st Sem IT students of Gayatri College Of Engg.........\n");
        ...
        while (true)
        {
            ...
            switch (choice)
            {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    StreamWriter sw = new StreamWriter("Class Marks 4-1.txt");
                    Console.SetOut(sw);
                    ind.AllStudents();
    ...
    private void IndividualStudent(string rollno)
    {
        try
        {
            ...
            //http://jntu.ac.in/results/results.php?htno=04131a1256&ecode=16441r04
            sr = GetStream("http://jntu.ac.in/results/results.php?htno=" + rollno + "&ecode=16441r04");
            stream = sr.ReadToEnd();
    ...
    private void AllStudents()
    {
        for (int i = 0; i < noStu; i++)
        {
            ...
            sr = GetStream("http://jntu.ac.in/results/results.php?htno=04131a" + (1200 + i + 1) + "&ecode=16441r04");
            stream = sr.ReadToEnd();
    ...
    private void DisplayDetails(int i)
    {
        ...
        if (stu[i].exists)
        {
            Console.WriteLine("{0,-25} {1,-15}  {2,-20}", stu[i].name, stu[i].roll,stu[i].fathName);
            Console.WriteLine("-------------------------------------------------------------------------------");
            ...
            for (int j = 0; j < 8; j++)
            {
                ...
                if (subj[j].Contains("LAB")) Console.WriteLine("{0,-6}", (stu[i].toteach[j] >= 30) ? "PASS" : "FAIL");
                else Console.WriteLine("{0,-6}", (stu[i].toteach[j] >= 40) ? "PASS" : "FAIL");
            }
            Console.WriteLine("                                                                 ==== =========");
    ...
    {
        int t;
        if (s.Contains("Name : --")) { stu.exists = false; return; }
        ...
        //Extract Father Name
        pos = s.IndexOf(fathName) + fathName.Length;
        s = s.Substring(pos);
