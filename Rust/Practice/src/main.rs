...
fn main() {
    ...
    *arr_refs[1] = 10;
    ...
    // Array implement Debug trait
    let mut arr = [1, 2, 3, 4];
    println!("{:?}", arr);

    // Array implements below for Vector
    // impl<T: Clone, const N: usize> From<&[T; N]> for Vec<T>
    let mut arr = [1, 2, 3, 4];
    let vec: Vec<i32> = (&arr).into(); // This will invoke the above From<&[T;N]> implementation
    let vec: Vec<i32> = arr.into(); // This will invoke From<[T;N]> implementation, which will consume the array if T is not copyable

    let mut arr = [1, 2, 3, 4];
    let vec: (i32, i32, i32, i32) = arr.into();
}
