...
{ char c;int k;
  ...
  while(!kbhit())
   ...
   {if(y[k]==c){ gotoxy(10,50); printf("RIGHT ");
		 gotoxy(1,1);delay(100);sound(420);delay(50);nosound();}
   else {gotoxy(10,50);printf("WRONG ");delay(100);gotoxy(1,1);x[(int)(y[k]-'a')]++;count++;}
   }
   }
   Graph(2);
   }
...
int main()
{
char *p[11],c;int ch,xx,yy,t1,t2;
// lessons
...
if(ch==1){ t1=time_();
	  ...
	  { clrscr();
	    ...
	    for(;*p[lesson]!='\0';)
	   {
	    ...
	    yy=wherey();
	    gotoxy(40,4);
	    printf("Letter to be typed is:%c Time left is %d:",*p[lesson],TIME-(t2-t1)%60);
	    gotoxy(xx,yy);
	    fflush(stdin);
	    t2=time_();
	    if(t2==(t1+TIME)%60){ printf("\n\n\nTIME IS UP.....");break;}
	    c=getch();
	    if(c=='0')exit(0);
	    if(c!='\b')printf("%c",c);
	    if(c!='\b')if(c!=*p[lesson]++){sound(420);delay(100);nosound();printf("\b"); x[(int)(*p[lesson]-'a')]++;p[lesson]--;count++;}
	   }
	   Graph(1);flushall();next_round();
	  }
	  else {printf("IMPROPER ARGUMENT.....!");sleep(3);clrscr();main();}
	 }
else if(ch==2){bk();Game();flushall();next_round();}
else if(ch==3){bk();help();}
else if(ch==4)exit(0);
