...
class harshaUDP
{
	...
	static void Main()
	{
		try{
			...
			localendpoint = new IPEndPoint(IPAddress.Parse(Dns.GetHostEntry("").AddressList[0].ToString()),8192);

			Console.WriteLine("Enter the  remote IP address...:");
			remoteIP = Console.ReadLine();
			remoteendpoint = new IPEndPoint(IPAddress.Parse(remoteIP),8291);
